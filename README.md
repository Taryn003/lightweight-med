...
- 基于 Qwen3-VL 构建胸片+病历多模态辅助诊断系统，支持结构化报告生成与医生在线编辑闭环。
- 在 IU 公开数据上完成 Teacher(4B) LoRA 与 Student(2B) 蒸馏，建立可复现训练-评估-部署流程。
- 构建 API/Web 双形态交付与性能监控，输出报告质量（ROUGE/BLEU）与部署指标（P50/P95、吞吐、显存）。
